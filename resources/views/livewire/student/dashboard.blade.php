...
>
    <div style="background:white;border-radius:16px;width:100%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.25);overflow:hidden;">

        {{-- Header popup --}}
        <div style="background:linear-gradient(135deg,{{ $isReentry ? '#92400E' : '#C0392B' }},{{ $isReentry ? '#78350F' : '#922B21' }});padding:20px 16px;text-align:center;position:relative;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;border-bottom:0.5px solid #E5E5EA;">
            <div style="padding:12px;text-align:center;border-right:0.5px solid #E5E5EA;">
                <div style="font-size:18px;font-weight:700;color:{{ $isReentry ? '#92400E' : '#C0392B' }};">{{ $selectedExam->duration }}</div>
                <div style="font-size:11px;color:#8E8E93;">menit</div>
            </div>
            <div style="padding:12px;text-align:center;">
                <div style="font-size:18px;font-weight:700;color:{{ $isReentry ? '#92400E' : '#C0392B' }};">{{ $selectedExam->classRoom?->name ?? 'Semua' }}</div>
                <div style="font-size:11px;color:#8E8E93;">kelas</div>
            </div>
        </div>

        {{-- Info reentry: tampilkan kapan keluar & sisa waktu --}}
        ...
        <div style="background:#FFFBEB;border-bottom:0.5px solid #FCD34D;padding:12px 16px;">
            <div style="font-size:12px;color:#78350F;line-height:1.7;">
                ...
                Anda keluar dari halaman ujian pada
                <strong>{{ $selectedSession->last_violation_at->format('H:i:s') }}</strong>.<br>
                @endif
                @php $remaining = $selectedSession->getTimeLeftSeconds(); $remMins = floor($remaining/60); $remSecs = $remaining % 60; @endphp
                Sisa waktu: <strong>{{ $remMins }} menit {{ $remSecs }} detik</strong>.<br>
                Masukkan <strong>token re-entry</strong> dari guru pengawas untuk melanjutkan.
            </div>
        </div>
        @endif
        ...
        <div style="padding:20px 16px;">
            <div style="text-align:center;margin-bottom:16px;">
                <div style="font-size:24px;margin-bottom:6px;">{{ $isReentry ? '🗝️' : '🔑' }}</div>
            ...
            <input
                ...
                style="width:100%;text-align:center;font-size:22px;font-weight:700;letter-spacing:0.25em;padding:14px 12px;border:2px solid {{ $tokenError ? '#C0392B' : '#E5E5EA' }};border-radius:10px;color:{{ $isReentry ? '#92400E' : '#C0392B' }};text-transform:uppercase;background:#F9FAFB;outline:none;font-family:inherit;box-sizing:border-box;"
            ...
            @if(!$isReentry)
            {{-- Peringatan hanya untuk mulai baru --}}
        ...
        <div style="padding:0 16px 20px;display:flex;gap:10px;">
            ...
            <button wire:click="submitToken" wire:loading.attr="disabled"
                style="flex:2;padding:12px;background:{{ $isReentry ? '#92400E' : '#C0392B' }};border:none;border-radius:10px;font-size:14px;font-weight:700;color:white;cursor:pointer;font-family:inherit;">
